...
const skills = [
  { label: 'React', state: 'Proven', strength: 88, tone: 'verified' },
  { label: 'Performance', state: 'Partial', strength: 61, tone: 'partial' },
  { label: 'JavaScript', state: 'Partial', strength: 46, tone: 'partial' },
  { label: 'Problem Solving', state: 'Unverified', strength: 0, tone: 'open' },
  { label: 'Communication', state: 'Unverified', strength: 0, tone: 'open' },
] as const;

const questions = [
  { source: 'Fixed opener', target: 'React', question: fixedOpeningQuestion },
  {
    source: 'Adaptive',
    target: 'Performance',
...
export function Scorecard({ onNewInterview }: ScorecardProps) {
  ...
  return (
    <section className="mx-auto w-full max-w-[1240px] py-4 sm:py-8 lg:py-10" aria-labelledby="scorecard-title">
      ...
      <div className="depth-panel relative overflow-hidden rounded-[1.65rem] p-5 sm:p-7 lg:p-9">
        ...
        <div className="relative grid gap-8 lg:grid-cols-[minmax(0,1.25fr)_minmax(22rem,0.75fr)] lg:items-end">
          <div>
            <div className="inline-flex items-center gap-2 rounded-full border border-accent/15 bg-accent/5 px-3 py-1.5 text-xs font-semibold text-[#087461]">
              <CheckCircle2 className="h-3.5 w-3.5" />
              Showcase scorecard · sample data
            </div>
            <h1
              id="scorecard-title"
              ref={headingRef}
              tabIndex={-1}
              className="display-type mt-5 max-w-[15ch] text-3xl font-semibold leading-[1.06] tracking-[-0.05em] text-foreground sm:text-5xl"
            >
              Evidence ready for recruiter review.
            </h1>
            ...
            <div className="flex items-end justify-between gap-4">
              <div>
                ...
                <p className="data-type mt-3 text-5xl font-semibold tracking-[-0.06em] text-foreground">
                  60<span className="text-xl text-primary">%</span>
                </p>
              </div>
      ...
      <div className="mt-5 grid gap-5 lg:grid-cols-[minmax(0,1.25fr)_minmax(20rem,0.75fr)]">
        <section className="depth-panel overflow-hidden rounded-[1.35rem]" aria-labelledby="skill-review-title">
          <div className="flex items-center justify-between gap-4 border-b border-border/70 px-5 py-4 sm:px-6">
            <div>
              <div className="flex items-center gap-2 text-primary">
                <Layers3 className="h-4 w-4" />
                <span className="data-type text-[10px] font-semibold uppercase tracking-[0.15em]">Skill review</span>
              </div>
              <h2 id="skill-review-title" className="mt-1.5 text-base font-semibold text-foreground">
                Evidence by requirement
              </h2>
            </div>
            <span className="data-type text-[10px] text-muted-foreground">Demo dataset</span>
          </div>
              ...
              const Icon =
                skill.tone === 'verified'
                  ...
                  : skill.tone === 'partial'
                    ...
                    : CircleDashed;
              return (
                <div key={skill.label} className="grid gap-3 py-4 sm:grid-cols-[10rem_minmax(0,1fr)_5.5rem] sm:items-center">
        ...
        <section className="depth-panel rounded-[1.35rem] p-5 sm:p-6" aria-labelledby="reliability-summary-title">
          ...
          <div className="mt-4 space-y-2">
            {['Relevance', 'Quality', 'Latency', 'Format'].map((gate) => (
              <div key={gate} className="flex items-center justify-between rounded-lg border border-[#0e9f84]/15 bg-[#0e9f84]/[0.045] px-3 py-2.5">
      ...
      <section className="depth-panel mt-5 overflow-hidden rounded-[1.35rem]" aria-labelledby="decision-trace-title">
        <div className="flex flex-wrap items-center justify-between gap-4 border-b border-border/70 px-5 py-4 sm:px-6">
          <div>
            <div className="flex items-center gap-2 text-primary">
              <Sparkles className="h-4 w-4" />
              <span className="data-type text-[10px] font-semibold uppercase tracking-[0.15em]">Decision trace</span>
            </div>
          ...
          {questions.map((item, index) => (
            <article key={item.question} className="interactive-surface min-w-0 p-5 sm:p-6">
              <div className="flex items-center justify-between gap-3">
                <span className="data-type text-[10px] font-semibold text-primary">TURN {String(index + 1).padStart(2, '0')}</span>
      ...
      <section className="depth-panel mt-5 rounded-[1.35rem] p-5 sm:p-6" aria-labelledby="evidence-excerpts-title">
        <div className="flex items-center gap-2 text-primary">
          <MessageSquareQuote className="h-4 w-4" />
          <span className="data-type text-[10px] font-semibold uppercase tracking-[0.15em]">Evidence excerpts</span>
        </div>
        ...
        <div className="mt-5 grid gap-3 lg:grid-cols-3">
          {scriptedDemoAnswers.map((answer, index) => (
            <blockquote key={answer} className="interactive-surface instrument-surface rounded-xl p-4 text-xs leading-5 text-[#40506b]">
              “{answer}”
              <footer className="data-type mt-3 text-[9px] font-semibold uppercase text-primary">
      ...
      <div className="mt-6 flex flex-col items-center justify-between gap-4 rounded-2xl bg-[#101828] px-5 py-5 text-white shadow-[0_20px_45px_rgba(16,24,40,0.18)] sm:flex-row sm:px-6">
        <div>
          ...
          <p className="mt-1 text-xs text-white/60">InterviewIQ surfaces evidence; the recruiter owns the decision.</p>
        </div>
        <Button onClick={onNewInterview} className="h-11 w-full justify-between rounded-xl bg-white px-4 text-xs font-semibold text-[#101828] hover:bg-[#f3f6fb] sm:w-auto sm:min-w-48">
